...
## 9. Create the package .desktop file
...
```bash
source=("$pkgname::git+https://github.com/<YOUR_USERNAME>/<YOUR_REPOSITORY>.git")
```
...
## 11. Set the minimum package permissions
...
## 14. Check that the .deb did not inherit paths from your machine
...
- In final packages, the `.desktop` file should be adjusted to use the final system path, just like in the Debian flow.
- If you prefer, you can create the `.desktop` directly inside `package()` instead of copying the raw template.

## 8. Create the .desktop file for Arch

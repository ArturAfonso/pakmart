...
class PermissionSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Text(
          '${section.index} ${context.l10n.permissionSectionTitle(section.title)}',
        ...
        Column(
          children: [
            for (var index = 0; index < section.entries.length; index++) ...[
              if (section.entries[index]
                  case final InstalledPermissionToggleData toggle)
                PermissionToggleCard(
                  data: toggle,
                  value: toggleValues[toggle.permissionKey] ?? toggle.enabled,
                  onChanged: (value) =>
                      onToggleChanged(toggle.permissionKey, value),
                  borderColor: borderColor,
                  titleColor: titleColor,
                  secondaryColor: secondaryColor,
                  isDark: isDark,
                )
              else if (section.entries[index]
                  case final InstalledPermissionTagsData tags)
                PermissionTagsCard(
                  data: tags,
                  borderColor: borderColor,
                  titleColor: titleColor,
